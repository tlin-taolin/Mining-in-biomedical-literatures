...
object NgramUtilities {
  ...
  def takeNgram(words: List[String], n: Int): List[(String, Int)] = {
    ...
    helper(words).map(x => (x.mkString(" "), x.length))
  }

  def takeNgrams(sent: List[String], n: Int): List[(String, Int)] = {
    /* Take each `nn`-gram for each sentence. Here, 1 <= `nn` <= n. */
    (1 until n).flatMap(ind => takeNgram(sent, ind)).toList
  }

  def splitNgram(lines: String, takeN: (List[String], Int) => List[(String, Int)], n: Int): (String, List[(String, Int)]) = {
    val (id, alines): (String, List[String]) = splitLines(lines)
    val ngram: List[(String, Int)] = alines.flatMap(s => takeN(s.split(" ").toList, n))
    (id, ngram)
  }
